...
function getMysqlConfigFromEnv() {
  return {
    ...
    database: process.env.MYSQL_DATABASE || "social_match",
...
async function ensureDatabaseExists(config) {
  const { database, ...serverCfg } = config;
  const conn = await mysql.createConnection({
    host: serverCfg.host,
    port: serverCfg.port,
    user: serverCfg.user,
    password: serverCfg.password,
    timezone: serverCfg.timezone,
    dateStrings: serverCfg.dateStrings,
    supportBigNumbers: serverCfg.supportBigNumbers,
